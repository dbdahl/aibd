#' Define an Indian Buffet Feature Allocation Distribution
#'
...
#'
ibp <- function(mass, x) {
  if ( missing(mass) || is.null(mass) || is.na(mass) || is.nan(mass) || !is.numeric(mass) || ( length(mass) != 1 ) ) stop("'mass' is misspecified.")
  mass <- as.double(mass)
  if ( missing(x) || is.null(x) || is.na(x) || is.nan(x) ) stop("'nItems' is misspecified.")
  if ( length(x) == 1L ) {
    nItems <- as.integer(x)
